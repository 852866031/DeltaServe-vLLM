...
| 2026-09-10 — truthful `[batch …] FT … buf=` under TP | — | `v1/core/sched/output.py` (`SchedulerOutput.finetune_admit_state`: the scheduler-side coordinator's `(admission_open, pending_backward, fill+reserved, capacity)` stamped per step after `reserve()`), `deltaserve/ft_scheduler.py`* (stamps it next to `finetune_record_timing`), `v1/worker/gpu_model_runner.py` (`_maybe_log_batch_exec` prints the stamp; falls back to the local coordinator only when no stamp is present — under TP the worker coordinator is a mirror with no buffer accounting, so the old read was always `buf=0/N`) |
| 2026-09-10 — chunked-prefill FT admission wedge | — | `deltaserve/ft_scheduler.py`* (`_current_step_features`: a running request still prefilling counts as its next chunk, not one decode token; `admit_ft_to_step`: FT capped by the step's remaining token budget; `schedule()`: admission flags re-derived from the FT tokens that actually scheduled — a dropped injection no longer leaves `admission_open=False` with an empty buffer; `has_requests`' exhausted diag prints the gate inputs; `_rollback_ft_step` logs its restore), `deltaserve/coordinator.py`* (none functional) |
...
| Known-issues sweep 2026-09-08 | — | `v1/core/sched/output.py`* (`finetune_retired_req_ids`), `deltaserve/ft_scheduler.py`* (collects retired / rolled-back FT ids, stamps them on the next SchedulerOutput), `v1/worker/gpu_model_runner.py`* (`_update_states` drops those requests' cached state + input-batch slot — the documented `self.requests` leak), `deltaserve/coordinator.py`* (`_check_backward_stuck`: warn once, then DISABLE finetuning when the child is dead or unresponsive for 60 s, tp=1 and relay paths), `v1/worker/gpu_worker.py`* (lm_head / embed / final-norm keys resolved on `.base_layer.`-normalized names — fixes the tied-`lm_head` Qwen3-0.6B child failing to build), `tests/test_config_loader.py`* (accept vLLM's offline snapshot-path rewrite of `model`) |
| mixed-fwd-cuda-graph (2026-09-15) — CUDA graphs for MIXED FT batches | `deltaserve/ft_save_op.py`, `tests/test_ft_save_op.py`, `configs/serving_config_finetuning_qwen3_0.6b_eagerft.yaml`, `MIXED_FWD_CUDA_GRAPH.md` | `deltaserve/accumulate.py`* (buffers get one scratch row; persistent `ft_src`/`ft_dst` index tensors + pinned ring; `install_save_points` / `arm_op_step` / `capture_state`; slot table), `model_executor/models/qwen3.py` (`maybe_save` call sites: q/k/v pre-norm, ctx, layer_in, resid_mid; `Qwen3DecoderLayer._dserve_save_points`), `model_executor/models/qwen2.py` (`Qwen2MLP.forward` gate_up, `Qwen2Model.forward` final_in), `forward_context.py`* (`BatchDescriptor.has_ft`; `ForwardContext.ft_save` + `set_forward_context(ft_save=)`), `v1/cudagraph_dispatcher.py` (**new file for the fork** — has_ft PIECEWISE key set up to `graph_ft_max_tokens`; `dispatch(has_ft=)` never matches FULL), `v1/worker/gpu_model_runner.py`* (`_ft_graph_step` decided in the mask build for mixed batches; `force_eager`/`skip_compiled` only for FT-only; `arm_op_step` instead of `begin_step`; `_dummy_run(has_ft=)` + capture set `ft_save`), `v1/worker/gpu_worker.py`* (`install_save_points`), `deltaserve/ft_scheduler.py`* (`_will_use_graph` with has_ft; regime split switch; admission predicts with the composition-derived regime), `deltaserve/estimator.py`* (`REGIME_FT_MIXED` behind `set_ft_mixed_split`), `deltaserve/profiling_batch_generator.py`* (FT-only shapes when split), `config/finetune.py`* (`graph_ft_batches`, `graph_ft_max_tokens`), `configs/serving_config_finetuning_qwen3_{14b_tp2,0.6b}.yaml`* (on), `configs/serving_config_finetuning_qwen3_14b_tp2_tbt45.yaml` (A/B), `config/vllm.py`* (`compute_hash` includes `FinetuneConfig.compute_hash` — the save call sites trace differently per flag), `eval-tp/analyze_step_trace.py` (`ft_mixed`), `eval-tp/compare_runs.py` (before/after table), `tests/test_merged_estimator.py`* |
| Estimator `decode_bwd` regime + rank-symmetric, run-ahead-bounded pause (2026-09-08, night) | — | `deltaserve/estimator.py`* (`StepFeatures.bwd`; 4th regime `decode_bwd` = decode-only with a backward in flight, decode design matrix, falls back to `decode_only` cold; prefill-carrying `bwd` samples are `contended` → excluded from every fit), `deltaserve/ft_scheduler.py`* (stamps `feats.bwd = coord.pending_backward`), `config/finetune.py`* (`backward_run_ahead_boundaries: int = 2`), `deltaserve/ft_meta.py`* (passes it to the child), `deltaserve/bwd_services/base.py`* (`_maybe_pause`: CUDA-event run-ahead ring + rank-symmetric gloo agreement; the two per-pause prints are gone), `deltaserve/bwd_services/common/tp.py`* (`backward_cpu_group()` — a gloo group over the children's rendezvous), `deltaserve/bwd_services/common/trainer.py`* (`_pause_group`; passes `pause_fn` to the head), `deltaserve/bwd_services/common/head.py`* (`head_backward(..., pause_fn)` — a boundary per vocab chunk in both passes), `tests/test_merged_estimator.py`* (`test_decode_bwd_regime`), `tests/test_tp_trainer_graph_nccl.py`* (run-ahead=2 + an always-set grant so the agreement runs; counter ignores `group=` collectives), `configs/serving_config_finetuning_qwen3_14b_tp2.yaml`* |
| Step trace (estimator validation) 2026-09-08 | `deltaserve/step_trace.py`, `tests/test_step_trace.py`, `eval-tp/analyze_step_trace.py` | `config/finetune.py`* (`step_trace_path`), `deltaserve/estimator.py`* (`select_regime`), `deltaserve/coordinator.py`* (`push_sample(..., extra)`), `deltaserve/ft_scheduler.py`* (`_trace_stamp` / `_trace_step` / `_trace_rollback`; admission records what it reasoned on), `v1/worker/gpu_model_runner.py`* (ring owner carries `(seq, t_exec, host_s, paused)`), `v1/engine/core.py`* (profiler drain unpacks the 5-tuple), `eval-tp/{launch_deltaserve,auto_benchmark_tp}.py` (`--step-trace`) |
...
### `vllm/deltaserve/step_trace.py` — `StepTraceWriter` (estimator validation)
**Function:** daemon-thread CSV writer for the per-step predicted-vs-actual trace
(`finetune.step_trace_path`). The scheduler formats a row where the runner's timing sample
is drained (~4 µs) and enqueues it; the thread owns the file, flushes once per second.
Column reference in the module docstring. Off (None) → no work at all.
**Used by:** `deltaserve/ft_scheduler.py` (`_trace_stamp` stamps `_ft_step_seq` on the
SchedulerOutput and keeps the scheduler-side context; `_trace_step` joins it with the
runner's `(seq, t_exec, host_s, paused)`; `_trace_rollback` writes pre-empted steps);
read by `eval-tp/analyze_step_trace.py`.

### `vllm/deltaserve/ft_save_op.py` — the graph-capturable activation save (mixed-fwd-cuda-graph)
**Function:** `torch.ops.vllm.dserve_save_rows(x, r, marker, slot)` — a custom op
(`direct_register_custom_op`, `mutates_args=["marker"]`) that gathers a fixed
`max_saved` rows of `x` (+ `r`) through the persistent source-index tensor and
`index_copy_`s them into the accumulator buffer for `slot` through the persistent
destination-index tensor, both read from `ForwardContext.ft_save` (never from the
graph). Opaque to Dynamo, so its Python branch on `ft_save` is evaluated at capture time:
plain graphs capture no kernels for it, `has_ft` graphs capture the gathers with the
persistent addresses, the compiled-uncaptured path evaluates it every step. The marker
mutation is what keeps the op alive — without a declared mutation an output-less op is
dead-code-eliminated by Dynamo and Inductor. `maybe_save(module, name, x, r)` is the
model-side call site (a no-op unless the accumulator installed a slot on the module).
**Used by:** `model_executor/models/{qwen3,qwen2}.py`, `deltaserve/accumulate.py`.
...
| `eval-arxiv/{fetch_arxiv,launch_arxiv,prefill_bench,trace_bench,plot_arxiv}.py` (+ `eval-arxiv/plots/`), `configs/serving_config_finetuning_qwen3_14b_tp2_arxiv.yaml` | 2026-09-10 long-context probe: parquet fetch of `ccdv/arxiv-summarization` (no `datasets` lib), server launcher over `eval-tp/launch_deltaserve.build_server_cmd` (`--co`, inference-only pinned to the v1 runner, `--max-num-batched-tokens`), TTFT-as-prefill probe (distinct article per request, `--gap`, `--conc`), Poisson-arrival trace replay (`--rps`, `--n`, prompt length ± jitter, `max_tokens` = abstract length), timeline / A-B plotter; YAML = TP=2 YAML with `max_model_len 16384` + `ttft_slo 3.0`. Results + the scheduler wedge it exposed in `eval-arxiv/README.md` |
