...
class EngineArgs:
    ...
    finetune_config: FinetuneConfig = get_field(VllmConfig, "finetune_config")
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        vllm_group.add_argument("--finetune-config", **vllm_kwargs["finetune_config"])
        vllm_group.add_argument(
    ...
    def create_engine_config(
        ...
        # [DeltaServe] When co-serving finetuning is enabled, default log_stats
        # OFF. vLLM attaches per-step scheduler_stats to the rank-0 frontend's
        # output stream; processing that on every engine step saturates that
        # frontend's event loop and stalls request accept/streaming (TTFT spikes
        # → FT admission backs off). Finetuning has its own engine-side metrics,
        # so the inference stat stream isn't needed. Mutating self here
        # propagates to every `engine_args.disable_log_stats` read (engine +
        # frontends), since create_engine_config runs before they're built.
        if self.finetune_config.enable_finetuning and not self.disable_log_stats:
            self.disable_log_stats = True
            logger.info(
                "[DeltaServe] enable_finetuning=True → disabling vLLM log_stats "
                "(the per-step scheduler_stats stream otherwise stalls the API "
