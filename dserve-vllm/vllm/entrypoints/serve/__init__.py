...
def register_vllm_serve_api_routers(app: FastAPI):
    ...
    # [DeltaServe] POST /start_finetuning (only when finetuning is enabled).
    from vllm.entrypoints.serve.finetune.api_router import (
        attach_router as attach_finetune_router,
    )

    attach_finetune_router(app)
