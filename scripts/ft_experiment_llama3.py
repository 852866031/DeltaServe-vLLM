#!/usr/bin/env python
"""Co-serving experiment harness (HTTP server + client) — Llama-3.
...
decision prints stream live, e.g.:
  ...
  [verify]   layer_in[0] vs embed[ids]: max|Δ|=... OK
  [verify]   RMSNorm(final_in) vs final_hidden: max|Δ|=... OK
...
Usage (inside the dserve-vllm conda env, with CUDA env from README.md):
    ...
    python ft_experiment_llama3.py --iterations 20 --interval 1.0 --max-tokens 10
"""
...
_DEFAULT_CONFIG = _ROOT / "configs" / "serving_config_finetuning_llama3.yaml"
_SERVED_NAME = "llama3"
...
def _start_finetuning(base_url: str) -> bool:
    """POST /start_finetuning to open FT admission (server launched with
    start_on_launch=false)."""
    req = urllib.request.Request(
        f"{base_url}/start_finetuning", data=b"", method="POST")
    try:
        with urllib.request.urlopen(req, timeout=10) as r:
            print(f"[experiment] start_finetuning -> HTTP {r.status}", flush=True)
            return r.status == 200
    except Exception as e:
        print(f"[experiment] start_finetuning failed: {e}", flush=True)
        return False
...
def main():
    ...
    # Hold FT admission closed at launch so we can exercise POST
    # /start_finetuning below (profiling at launch is unaffected).
    ...
    try:
        ...
        print("[experiment] server ready", flush=True)

        # Open FT admission via HTTP before the first request (the server was
        # launched with start_on_launch=false).
        _start_finetuning(base_url)

        print("[experiment] firing requests", flush=True)
